...
nGPI = 1;
K = zeros(2 , 4 , nP);
K(: , : , 1) = place(A , B , [0.7 1.8 0.006 0.09]); % first feasibble policy
...
disp('The strategic Cost of GPI method is:')
disp(0.5 * x(:,1)' * P{j+1} * x(:,1))
%% plot results
o = 200; %number of iteration
K11 = K(1 ,1 ,:) ; K11 = K11(:) ;
K12 = K(1 ,2 ,:) ; K12 = K12(:) ;
K13 = K(1 ,3 ,:) ; K13 = K13(:) ;
K14 = K(1 ,4 ,:) ; K14 = K14(:) ;
...
subplot(241)
plot(1:j+1 , K11(1:j+1) ,LineWidth=1.5);xlabel('Iteration');ylabel('K11') ; hold on ; grid on;xlim([0 j+1]);ylim([-5 20]);

subplot(242)
plot(1:j+1 , K12(1:j+1) ,LineWidth=1.5);xlabel('Iteration');ylabel('K12') ; hold on ; grid on;xlim([0 j+1]);ylim([-5 20]);

subplot(243)
plot(1:j+1 , K13(1:j+1) ,LineWidth=1.5);xlabel('Iteration');ylabel('K13') ; hold on ; grid on;xlim([0 j+1]);ylim([-5 20]);

subplot(244)
plot(1:j+1 , K14(1:j+1) ,LineWidth=1.5);xlabel('Iteration');ylabel('K14') ; hold on ; grid on;xlim([0 j+1])

subplot(245)
plot(1:j+1 , K21(1:j+1) ,'r',LineWidth=1.5);xlabel('Iteration');ylabel('K21') ; hold on ; grid on;xlim([0 j+1]);ylim([-5e5 0.5e5]);

subplot(246)
plot(1:j+1 , K22(1:j+1) ,'r',LineWidth=1.5);xlabel('Iteration');ylabel('K22') ; hold on ; grid on;xlim([0 j+1]);ylim([-7e4 1e4]);

subplot(247)
plot(1:j+1 , K23(1:j+1) ,'r',LineWidth=1.5);xlabel('Iteration');ylabel('K23') ; hold on ; grid on;xlim([0 j+1]);ylim([-15e6 0.5e6]);

subplot(248)
plot(1:j+1 , K24(1:j+1) ,'r',LineWidth=1.5);xlabel('Iteration');ylabel('K24') ; hold on ; grid on;xlim([0 j+1]);ylim([-7e5 0.5e5]);


Fig2 = figure(2);
Fig2.Color = [0.9 0.9 0.9];
plot(1:nP,x(1,1:nP),LineWidth=1.5)
hold on
plot(1:nP,x(2,1:nP),LineWidth=1.5)
hold on
plot(1:nP,x(3,1:nP),LineWidth=1.5)
hold on
plot(1:nP,x(4,1:nP),LineWidth=1.5)
hold on
grid on
xlabel('Iteration')
ylabel('States')
